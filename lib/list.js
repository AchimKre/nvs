...
function getNodejsRemoteVersionsAsync(remoteName, remoteUri) {
	...
	return new Promise((resolve, reject) => {
		console.log("Download: " + remoteIndexUri);
		let options = remoteIndexUri;
		if (process.env.NVS_PROXY) {
			let proxy = process.env.NVS_PROXY.split(':');
			options = {
				host: proxy[0],
				port: proxy[1],
				path: remoteIndexUri,
				headers: {
					Host: url.parse(remoteIndexUri).hostname
				}
			};
			console.log("Use proxy config: ", options);
		}
